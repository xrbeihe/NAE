...
- **新增**：体质（有则显）、物品（非空时显）
- **不再含位置**：位置不进代码层（面板不渲染），主角位置只在信息栏【主角动态】段里由模型写
- 性格加回
- 全部一行，` ｜ ` 分隔，CSS `white-space: pre-wrap` 自动折行

### 4. Prompt 文档更新（`prompt_builder.py:124-144`）
...
| 文件 | 改动 |
|---|---|
| `backend/ane/game_engine.py` | Step 15 重写 + Step 16 面板精简 + 扩展系统写回 |
| `backend/ane/modules/prompt_builder.py` | PlayerContext/NPCContext 加 `extensions` 字段 + 渲染 + Prompt 文档 |
