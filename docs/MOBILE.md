...
### NPC 总库（主页 `#home-npc-table`）

`renderNpcTable` 按视口宽度切换布局：
- **≤768px**：卡片布局——每个 NPC 一张卡，「名称 + 编辑/删除按钮」一行，下面 `📍身份 / ⚡修为 / 🏷标签` 横向排布（`flex-wrap` 换行）。多张卡一屏可见，中文无竖排
- **>768px**：保留 5 列表格布局
- 容器 `min-height: 120px` 防空表时区域坍缩（曾出现手机端「总库区域直接消失」）
