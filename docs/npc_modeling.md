...
> 版本：1.2
>
> **状态更新（2026-07-31）**：⭐ 前端入口（局内建模）已关闭。标记 turn 触发路径当前无 UI 触发；主动建模路径仍由 NPC 总库（`/npcs/library`）保留调用建模能力。
...
> ⚠️ ⭐ 前端入口已关闭（2026-07-31），此触发路径当前不可用。建模能力本身通过 NPC 总库（`/npcs/library` 新建建模）仍在使用。

---

## 触发条件

- **标记 turn（已关闭）**：~~玩家在前端勾选「⭐ 重要人物」复选框发送输入，`mark_important_npc=True` 传入后端~~
  - ~~后端 `process_turn()` 识别 NPC 名，设置 `is_important=True`，写入 `pending_debut` 标记~~
  - ~~不在此刻建模——建模走独立 API~~
- **主动建模**：NPC 总库「新建建模NPC」通过 `POST /npcs/library` 调用建模能力，无需勾选 ⭐
...
| **v1.2（当前）** | **独立 API（2 步 REST）** | ⭐ 前端入口已关闭；建模能力由 NPC 总库（`/npcs/library`）调用 | ❌ 建模不产叙事，出场时生效 |
