...
# ── tkinter GUI（Python内置，无需额外安装） ──────────────────

import tkinter as tk
from tkinter import scrolledtext, messagebox

# 导入工具核心
from ane.tools.nsfw_harvest import fetch_page, extract_content, llm_extract, merge_to_templates

# 配置
API_KEY = ""
API_URL = ""
_NSFW_PATH = _PROJ / "backend" / "ane" / "content" / "nsfw_templates.json"
_UNDERAGE_PATH = _PROJ / "backend" / "ane" / "content" / "underage_templates.json"
...
class HarvestGUI:
    def __init__(self):
        ...
        self.select_frame.pack_forget()  # 初始隐藏
    ...
    def start_harvest(self):
        ...
        if not API_KEY:
            messagebox.showerror("错误", "未配置 DEEPSEEK_API_KEY，请检查 .env 文件")
            return
    ...
    async def _process_url(self, url, direct):
        ...
        if len(text) < 100:
            self.append_result("⚠️ 正文太短，可能提取失败\n")
            return
        ...
        if not extracted.get("has_nsfw"):
            self.append_result("未检测到 NSFW 内容\n")
            return

        # 显示提取结果摘要
        positions = extracted.get("positions_used", [])
        dialogue_count = len(extracted.get("dialogue_lines", []))
        mood = extracted.get("mood", "")

        self.append_result(f"体位: {positions}\n")
        self.append_result(f"对话: {dialogue_count} 条\n")
        self.append_result(f"氛围: {mood}\n")

        # 入库
        is_underage = extracted.get("has_underage", False)
        target_name = "underage_templates.json" if is_underage else "nsfw_templates.json"
        ok = merge_to_templates(extracted, source_url=url)
        if ok:
            # 读取对应库的最终状态
            target_path = _UNDERAGE_PATH if is_underage else _NSFW_PATH
            d = json.loads(target_path.read_text(encoding="utf-8"))
            total = sum(len(v) for v in d.get("sensory_bundles", {}).values())
            label = "🔞 未成年" if is_underage else "NSFW"
            self.append_result(f"\n✅ 写入成功 → {target_name}\n")
            self.append_result(f"当前{label}库: positions={len(d['positions'])} dialogue_dom={len(d['dialogue']['dominant_male'])} dialogue_sub={len(d['dialogue']['submissive_female'])} sensory={total}\n")
        else:
    ...
    def _search_articles_go(self, raw):
        ...
        t = threading.Thread(target=self._run_batch,
                             args=(selected, self._search_direct), daemon=True)
        t.start()
