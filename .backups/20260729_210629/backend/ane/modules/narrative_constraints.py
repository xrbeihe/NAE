...
class NarrativeConstraints:
    ...
    def get_context_constraints(
        ...
        Merges global rules with context-specific constraints derived from
        player state and active NPC data.
        """
        ctx = ConstraintSet(
            hard=list(self._global_hard),
            soft=list(self._global_soft),
            triggers=list(self._global_triggers),
        )
        ...
        ctx.hard.append(
            f"玩家当前修为为{player_cultivation}，不得表现出超出此修为的能力。"
            f"修为提升需要时间和契机，不能突然暴涨。"
        )
