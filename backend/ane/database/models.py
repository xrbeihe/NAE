...
class NPC(Base):
    ...
    source_user_npc_id = Column(String, nullable=True)         # references UserNPC.id for imported NPCs
...
# ── User NPC Library (跨世界总库) ──────────────────────────

class UserNPC(Base):
    """User-level NPC library — model data shared across all sessions."""
