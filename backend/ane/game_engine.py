...
节奏建议：根据场景性质选择——Type 1（刺激插曲）在一轮内闭环，给出完整的挑逗→前戏→进入→多次高潮/射精→事后的刺激过程；Type 2（情节性性爱）可跨轮次推进，每轮有阶段性闭环和推进结果。无论哪种模式，每轮都必须有推进感。"""
    ...
    async def _handle_system_command(
        ...
        """Handle system commands /status, /help, /facts, /addfact."""
        if cmd == "system_status":
            return await self._cmd_status(db, session_id)
        elif cmd == "system_help":
            return self._cmd_help()
        elif cmd == "system_list_facts":
            return await self._cmd_list_facts(db, session_id)
        return TurnResult(
            is_system_command=True,
            system_response="未知命令。可用命令：/status, /help, /facts, /addfact <内容>",
        )

    async def _cmd_status(self, db, session_id) -> TurnResult:
        player = await pm.get_by_session(db, session_id)
        if not player:
