...
# 面板已经把 姓名/性别/年龄/身份/能力/性格… 列成权威版本；但 LLM 常会在
# 【主角动态】里再写一遍（例如「北荷茶光：查克拉消耗过半，精神紧绷 ｜身份：水忍中忍」）。
# 上一轮信息栏是原样回喂的，于是这份回声逐轮固化。这里按**字段名**剥离：
# 只删「字段：值」中字段已出现在权威面板里的片段，保留真正的动态内容（状态/当前行动/位置…）。
# 位置是例外：它不在权威面板里（程序不跟踪位置），所以信息栏里的「位置：…」是有效信息，保留。
...
# 注意：位置已经不在这个名单里——位置是**叙事/信息栏**层的概念（写在【主角动态】段里），
# 只是不进代码层，所以信息栏里的「位置：…」必须保留，不能当回声剥掉。
_ALWAYS_ECHO_KEYS: set[str] = set()
...
def strip_dynamic_field_echoes(text: str, panel_text: str) -> str:
    ...
    已经完全从角色数据里移除的字段，即便面板不再展示也不该出现在信息栏里
    （当前为空——位置属于信息栏该写的内容）。
    """
    if not text:
        return text
