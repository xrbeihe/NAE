...
- **每一轮都必须输出**本轮叙事中出现的、与玩家有互动的所有有名有姓NPC，**包括之前已经输出过的角色**。
- 每条输出 name（NPC姓名）+ description（**当前最新的**关系+互动简述，**必须叠加所有已知的关系层次**）。
- 关键：**不要只写本轮新发生的关系，而是把该角色与玩家已知的所有关系层次一同列出**。
  - 正确示例：如果林慕萱先是玩家的母亲，后来发生了乱伦关系，则每轮输出都应写：`{"name": "林慕萱", "description": "亲生母亲，已有乱伦关系", "type": "母子/乱伦", "affinity": 70}`
  - 错误示例：只写 `{"description": "发生了关系"}`——这样会丢失"母亲"这个基础关系。
- 总之，LLM在填写 `player_relationships` 时需要基于记忆中的已知关系填写，不能只基于本轮叙事。
  description 字段应叠加所有已知关系，用逗号或斜杠分隔。
