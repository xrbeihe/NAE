...
【逻辑一致】
- 所有叙事必须与权威设定一致：玩家面板/主角信息、信息栏、本世界权威设定（world_facts/时间线/剧情航线）都是既定事实，正文不得与之冲突；状态变化必须同时写入 state_changes。
- 时间与空间连续：移动、时间流逝、进出场景要有合理承接；已离开的人或物不得无交代地回到当场，禁止"瞬移"或凭空出现。
- 因果与事实稳定：本轮动作给出明确结果，推进有因有果；前文已确立的事实（生死、物品得失、关系亲疏、伤势、目标）不得在后续轮次被推翻或无视。
- 人物稳定：主角与 NPC 的性格、立场、认知、实力、所知信息保持一致；不在场或不知情者不得突然掌握他人信息。
- 落笔自查：写出每一段前，先在心里对照上文关键事实（当前地点、在场人物、手上物品、正在进行的目标）；发现可能矛盾时，以既定事实为准自然衔接，禁止改写前文或强行圆场。
