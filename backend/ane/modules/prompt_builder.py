...
NOTE: 推荐行动 / 附近人物已并入 info_panel（信息栏），随【上一轮信息栏】回喂下一轮
（模型据此保证"推荐与上轮明显不同"）；不再是独立的 JSON 字段。
"""
...
```json
{
  ...
  "info_panel": "所有信息类内容的唯一去处：主角动态 / 交互人物 / 附近人物 / 推荐行动（见下）"
}
```
...
- **段标题固定照抄这四个：【主角动态】【交互人物】【附近人物】【推荐行动】**——不要改名、不要用主角名/人物名当标题（如「【无名忍者】」「【当前交互人物】」都是错的），也不要自创段名。**标题必须独占一行，内容从下一行开始**（绝不要写成「【主角动态】名字：状态：…」这样挤在同一行）。
- 【主角动态】写**动态**内容（状态/伤势/心情/当前行动等）**＋主角的位置**。**位置是主角的基本信息，每轮都要写在这一段里**（位置只进信息栏、不进程序：不要用 state_changes 写它，也不要为位置造事件）。**绝不重复主角面板里已有的静态字段**：姓名、性别、年龄、身份、能力、血继限界、性格、宗门——一个都不要写。格式如「主角名：状态：查克拉消耗过半，精神紧绷 ｜当前行动：正赶往杉谷村口 ｜位置：林之国·杉谷村口」，无显著变化也输出最小一行（位置始终要有）。
- 【交互人物】本轮与主角有实质互动的角色，最多 1-2 位。每位**独占一行**，**每一个维度都必须带字段名**（写成无标签的一串描述=不合格），字段与顺序固定：「姓名 ｜ 身份：… ｜ 性格：… ｜ 外貌：… ｜ 行为：… ｜ 状态：… ｜ 装备：…」。反例（不合格）：「白 ｜ 雾隐叛忍的随从 ｜ 温柔顺从 ｜ 面容清秀 ｜ 一直未醒 ｜ 昏迷 ｜ 无」。有建模数据的角色，外貌与性格要写足（各 2-3 句具体特征，不要「清秀」这种四个字的标签词）；没有的维度写「无」。路人角色不放这里。
- 【附近人物】本轮场景里出现的其他人物：**有谁、几位，由你按当前场景自行决定**（不做人数或性别限制，也不要求凑数）。每位**独占一行**，**每个维度都要带字段名**（无标签的一串描述=不合格），顺序固定：「姓名 ｜ 身份：… ｜ 外貌：… ｜ 正在做什么：…」；一群人同行算一位（姓名处写清群体名与成员，如「第七班（旗木卡卡西、漩涡鸣人、宇智波佐助、春野樱）」）。玩家点名的、与其有重要关系的 NPC 必须列入。
...
- **禁止复述主角面板已有内容**：姓名/身份/能力/技能/物品/栏目（如「技能栏·XX」）已由权威主角面板列出，任何形式（分节标题、清单、表格）都不要再写一遍——重复会让每一轮上下文翻倍膨胀。若【上一轮信息栏】里出现了这类分节，本轮直接删掉，不要照抄。
...
- 你负责叙事：环境、动作、心理、对话都在 narrative 字段中。state_changes 只标记数据库需要记录的事实变更。
- 文风贴合世界观，信息密度要高，节奏明快。多用短句和动作推进。
  {word_count_rule}
- 避免"值得注意的是""综上所述"等AI句式。
- ❗ 严禁向玩家反问/征询下文（"你准备怎么做？""你觉得呢？""接下来，你打算？"等均属此类），
  禁止在正文末尾停顿等玩家（"你站在门口，不知道要不要进去。""你犹豫了。"）。
- 省略号统一使用三个半角句点...  
- 正文必须分段输出：每个自然段落之间留一个空行（\\n\\n），一段约 2-3 个完整句子。严禁把整段叙事挤成无换行的长文本。
...
【逻辑一致】
- 所有叙事必须符合已知：信息栏、用户设定都是既定事实，正文不得与之冲突。
- 时间与空间连续：移动、时间流逝、进出场景要有合理承接；已离开的人或物不得无交代地回到当场，禁止"瞬移"或凭空出现。
- 因果与事实稳定：正文内容不得前后矛盾，前文已确立的事实（生死、物品得失、关系变化、伤势、目标）不得在后续轮次被无视。
- 人物稳定：主角与 NPC 的所知信息保持一致，不在场或不知情者不得突然掌握他人信息。
- 落笔自查：写出每一段前，先在心里对照上文关键事实（时间人物地点动作等），发现可能矛盾时，以既定事实为准自然衔接。
...
【输出格式·本世界观特定】
- 信息栏【推荐行动】段的推荐内容多样化时涵盖修炼、社交、探索、任务等不同类型。第一轮如果玩家在宗门内，推荐行动应围绕宗门场景展开（拜访管事、接取任务、熟悉环境等），不要推荐城市相关行动。
...
class PromptBuilder:
    ...
    def _build_player_block(self, ctx: PromptContext) -> str:
        ...
        if p is not None:
            ...
            # 注意：这里**不再**渲染 `extension:` 行。栏目内容已由权威主角面板的
            # 「扩展：」项承载（panels.py，每轮随【上一轮主角信息】进 prompt），
            # 两处同时渲染等于同一批数据在同一个 prompt 里出现两遍——既浪费 token，
            # 又会让 LLM 误以为需要在自己的 info_panel 里再复述一遍（重复固化）。
            return "\n".join(lines)
