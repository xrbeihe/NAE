...
- 【主角动态】主角的静态信息（姓名/性别/年龄/身份/位置/能力/性格等）在权威主角面板中已有，**不要重复列出**；只写动态状态，格式如「主角名：状态：查克拉充足，精神平稳 ｜位置：…」，无显著变化也输出最小一行。
- 【交互人物】本轮与主角有实质互动的角色，最多 1-2 位。每位按「姓名｜身份｜性格｜外貌｜行为｜状态｜装备」七维单行输出，有建模数据则外貌适当详细。路人角色不放这里。
- 【附近人物】3 位场景路人（1 男 2 女）作氛围点缀，每位按「姓名｜身份｜外貌｜正在做什么」单行输出；玩家输入中点名的、与其有重要关系的 NPC 必须列入且不计名额。
