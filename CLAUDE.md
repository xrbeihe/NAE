...
### 🗺️ 移除所有角色的「位置」（主角 + NPC，含重要人物）
- **决定**：位置不再作为角色的数据/约束——主角在哪由**最近对话与玩家输入**自然决定（叙事里交代），
  NPC 的位置更是无关紧要。程序不再维护、不再注入、不再据此判定谁在场
- **提示词**：删掉玩家块的「具体位置」（含"未设定→用 location_change 确立"）、场景块的「位置层级/具体位置」、
  NPC 渲染的「位置：」与行为链里的位置项；state_changes 的类型清单与用法里删掉 `location_change`；
  【主角动态】规则改成「位置不需要写（程序不再跟踪任何角色的位置）」；npc_status 说明去掉"位置"
...
- **信息栏去回声**：前后端各加一份**固定剥离名单**（位置/地点/所在地/location）——面板不再展示位置后，
  仍要保证旧会话/旧输出里的「｜位置：…」被剥掉
- **保留**：DB 列（`players.location`/`attributes.location_hierarchy`/`NPC.location`）与
  ...
  （那是氛围，不是"谁在哪"）
- **测试**：+4（运行时 prompt 无位置行 / 6 包面板无位置字段 / 在场判定改用提到+重要+存活 / 端到端位置回声）；
  全量 **303 通过**
- **复查时又清掉的残留**（这些地方还在"教 LLM 位置"，会与上面的设计打架）：
