...
### 🧾 信息栏排版修复（段标题独占一行 + 【交互人物】带字段名）
- **症状**（实测截图）：① 并进主角面板的动态行前面挂着「【主角动态】」标题残留
  （`【主角动态】北荷茶光：状态：…`），观感像面板里嵌了第二个标题；② 【交互人物】写成
  「白 ｜ 雾隐叛忍的随从 ｜ 温柔顺从 ｜ 面容清秀 ｜ 一直未醒 ｜ 昏迷 ｜ 无」——七维顺序对但**没有字段名**，读不出哪段是什么
- **根因 1（提示词）**：info_panel 规则只说"各段用【…】小标题起头"，没要求**标题独占一行** →
  模型把标题和内容挤在同一行；而前后端的段边界识别都要求"标题单独成行"，于是这行被当成裸动态行原样保留
  （更糟的是：挤在一行的「【交互人物】白 ｜ 身份：…」会被当成主角动态段的**一部分**，
  段内 `身份：`/`性格：` 与权威面板字段重名 → 被回声剥离误删）
- **根因 2（提示词）**：【交互人物】的格式只写了「姓名｜身份｜性格｜外貌｜行为｜状态｜装备」这种
  **无标签**的占位，模型自然照抄成无标签描述
- **修法（提示词，两份副本）**：
  - 「段标题固定照抄这四个」那条补：**标题必须独占一行，内容从下一行开始**（明令禁止「【主角动态】名字：…」）
  - 【交互人物】改成：每位独占一行，**每一个维度都必须带字段名**（无标签的一串描述=不合格），
    给出正例格式「姓名 ｜ 身份：… ｜ 性格：… ｜ 外貌：… ｜ 行为：… ｜ 状态：… ｜ 装备：…」+
    反例「白 ｜ 雾隐叛忍的随从 ｜ 温柔顺从 ｜ 面容清秀 ｜ …」；有建模数据的角色外貌/性格各写 2-3 句具体特征
  - 【附近人物】同步规范化：每位独占一行、每维带字段名「姓名 ｜ 身份：… ｜ 外貌：… ｜ 正在做什么：…」，
    并说明**一群人同行算一位**（姓名处写清群体名与成员，如「第七班（旗木卡卡西、漩涡鸣人…）」）；
    人数/性别仍不做限制（上一条约束已取消）
- **修法（引擎，`panels.normalize_section_headings()`）**：把四段标题强制断成独占一行
  （标题前有内容 → 空行；标题后紧跟内容 → 换行），在 **Step 17 落库前**与**回喂前**各跑一遍，
  **顺序必须在回声剥离之前**（这是这条修复的关键：先规整，段边界才认得对，段内字段才不会被误删）
- **修法（前端）**：`app.html` 新增 `_normalizeInfoHeadings()`（与后端同规则，`addInfoPanel` 里
  显示前跑一遍 → 旧会话里已存下的挤行文本也能立刻显示正常）+ `_stripDynHead()` 兜底剥掉
  「【主角动态】名字：状态：…」的标题残留
- **保留的工具函数**：`panels.strip_glued_dynamic_heading()`（剥挤在一行的【主角动态】标记，防御性）
- **测试**：`test_prompts.py` +3（`test_normalize_section_headings_unit` /
  `test_glued_headings_normalized_before_storing` 端到端：挤行输入 → 落库已断开且段内字段未丢 /
  `test_prompt_interacting_person_requires_field_labels` 锁提示词口径）；JS 侧用**用户实际那一轮的数据**
  跑前端真实函数 → 10 项断言全 PASS；全量 **303 → 306 通过**

### 🗺️ 「位置」退出代码层（主角位置仍写在信息栏里）
- **决定**：位置不再是角色的**数据/约束**——不进 DB、不进 prompt 注入、不参与在场判定。
  主角在哪由**最近对话与玩家输入**自然决定；**位置作为主角的基本信息，每轮由模型写在
  信息栏的【主角动态】段里**（玩家看得见、下一轮原样回喂），NPC 的位置则无关紧要
- **提示词**：删掉玩家块的「具体位置」（含"未设定→用 location_change 确立"）、场景块的「位置层级/具体位置」、
  NPC 渲染的「位置：」与行为链里的位置项；state_changes 的类型清单与用法里删掉 `location_change`；
  【主角动态】规则改为「写动态内容 **＋主角的位置**；位置只进信息栏、不进程序（不要用 state_changes 写它）」；
  npc_status 说明去掉"位置"（NPC 位置不必写回）
- **在场判定**（`retrieval_engine`，位置彻底退出）：① 重要人物始终在场；② 名字出现在
  「本轮玩家输入 + 最近 3 轮对话 + 上一轮信息栏」里的 NPC 在场（`is_name_mentioned` 支持后缀简称
  「路飞」↔「蒙奇·D·路飞」，前缀不误伤）；③ 死亡 NPC 不注入；④ 上限 8，重要人物优先。
  顺带修掉三个老问题：空位置的路人"永远在场"、同村即在场、死者仍在场
- **引擎**：`location_change` 不再写回（只记日志，解析器仍认该类型 → 老输出不报错）；NPC 创建的 3 处
  （重要人物/关系数组/手动建模）不再传 `location`；`npc_status` 里 `field="location"` 被忽略
- **面板/包**：6 个包的 `panel.json` 删除「位置」字段；xianxia `ui.json` 角色卡的「初始位置」行删除；
  sanguo 包 `system_prompt.txt` 与 `pack_generator` 生成模板里的 `location_change` 说明删除
- **信息栏不再剥位置**：前后端各有一份回声剥离逻辑（字段级去重 + 固定剥离名单）——位置**从剥离名单里去掉**
  （`_ALWAYS_ECHO_KEYS` 清空、前端 `_panelHasField` 同步），因为位置已经不在权威面板里，信息栏里的「｜位置：…」是唯一来源，剥掉就没位置了
- **保留**：DB 列（`players.location`/`attributes.location_hierarchy`/`NPC.location`）与
  `location_change` 解析白名单保留兼容，但不再读写；场景块的**环境描写/时间/不在场相关人物**保留
  （那是氛围，不是"谁在哪"；新会话该行常为空，老会话残留位置仍可用于氛围查询）
- **测试**：+4（运行时 prompt 无位置注入 / 6 包面板无位置字段 / 在场判定改用提到+重要+存活 /
  端到端：位置保留在信息栏且 `location_change` 不写库）；全量 **303 通过**
- **复查时又清掉的残留**（这些地方还在"教 LLM 用位置做状态"，会与上面的设计打架）：
  - **6 个包的 `constraints.json`**：每轮注入的硬约束里那条「NPC只能在自身当前位置出现…不能突然现身」整行删除；
    状态一致性约束里的「位置」一并去掉（`narrative_constraints.py` 的引擎兜底默认值、`pack_generator` 的生成模板同改）
  - `memory_manager` 与 `game_engine` 后台摘要的短记忆指令「概括**玩家位置**和当前意图」→「概括玩家当前意图与正在做的事」
  - `player_manager._pick_start_location` 的 docstring（还写着"由 location_change 确立"）
  - `api/routes.py` 角色卡渲染里 location 行的默认 label「初始位置」→「信息」
  - `game_engine` 无面板兜底渲染的「位置：」行、`/status` 命令输出的「位置：」一并去掉（位置不进代码层，报了只会是"未知"）
  - 复查结论：6 包组装后的系统提示词、约束、面板里都不含位置注入；提示词里唯一提到位置的地方是
    **「位置写在信息栏【主角动态】段」**这条写作要求本身
